...
c     =====================================================
     ...
     &               ylower,zlower,dx,dy,dz,q,maux,aux,t,dt,mthbc,inpp)
c     =====================================================
c
...
c
	...
	real*8, dimension (-1:(mx+2),-1:(my+2)) :: inppcur
...
c     # zero-order extrapolation:
      do 515 kbc=1,mbc
         do 515 i = 1-mbc, mx+mbc
            ...
            q(i,j,1-kbc,1)=aux(i,j,1-kbc,1)+
     &           (q(i,j,1,1)-aux(i,j,1,1))*
     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
            q(i,j,1-kbc,2)=aux(i,j,1-kbc,2)+
     &           (q(i,j,1,2)-aux(i,j,1,2))*
     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
            q(i,j,1-kbc,3)=aux(i,j,1-kbc,3)+
     &           (q(i,j,1,3)-aux(i,j,1,3))*
     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
            q(i,j,1-kbc,4)=q(i,j,1,4)*
     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
            q(i,j,1-kbc,5)=aux(i,j,1-kbc,5)+
     &           (q(i,j,1,5)-aux(i,j,1,5))*
     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
            do 515 m = 6, meqn
...
c	endif
	...
	do 101 i=-1, mx+2
	do 102 j=-1, my+2		
		inppcur(i,j) = inpp(int(t*8)*336+k)
		k=k+1
102	continue
101	continue
...
c	     This is for 3
c            q(i,j,1-kbc,3)=aux(i,j,1-kbc,1)*inppcur(i,j)
     ...
     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
        	...
        	q(i,j,1-kbc,4)=aux(i,j,1-kbc,1)*inppcur(i,j)

c		now I added to 4
c            q(i,j,1-kbc,4)=q(i,j,1,4)*
c	     &           (aux(i,j,1-kbc,1)/aux(i,j,1,1))**(.5)
...
c     # negate the normal velocity:
c      do 536 kbc=1,mbc
c         do 536 i = 1-mbc, mx+mbc
c            do 536 j = 1-mbc, my+mbc
c               q(i,j,1-kbc,4) = -q(i,j,kbc,4)
c	  536    continue
      go to 599
...
c     # zero-order extrapolation:
      do 615 kbc=1,mbc
         do 615 i = 1-mbc, mx+mbc
            ...
            q(i,j,mz+kbc,1)=aux(i,j,mz+kbc,1)+
     &           (q(i,j,mz,1)-aux(i,j,mz,1))*
     &           (aux(i,j,mz+kbc,1)/aux(i,j,mz,1))**(.5)
            q(i,j,mz+kbc,2)=aux(i,j,mz+kbc,2)+
     &           (q(i,j,mz,2)-aux(i,j,mz,2))*
     &           (aux(i,j,mz+kbc,1)/aux(i,j,mz,1))**(.5)
            q(i,j,mz+kbc,3)=aux(i,j,mz+kbc,3)+
     &           (q(i,j,mz,3)-aux(i,j,mz,3))*
     &           (aux(i,j,mz+kbc,1)/aux(i,j,mz,1))**(.5)
            q(i,j,mz+kbc,4)=q(i,j,mz,4)*
     &           (aux(i,j,mz+kbc,1)/aux(i,j,mz,1))**(.5)
            q(i,j,mz+kbc,5)=aux(i,j,mz+kbc,5)+
     &           (q(i,j,mz,5)-aux(i,j,mz,5))*
     &           (aux(i,j,mz+kbc,1)/aux(i,j,mz,1))**(.5)
            do 615 m = 6, meqn
