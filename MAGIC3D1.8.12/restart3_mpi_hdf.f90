...
!      call h5pset_deflate_f(plist_id, 6, ierr)
     ...
     ! call h5aopen_name_f(dataset_id,"Parameters", attr_id, ierr)
     ...
     ! data_dims(1) = 17
     ! call h5aread_f(attr_id, atype_id, attr_dataread, data_dims, ierr)
     
     ! tinitial = attr_dataread(12)
     
     !     Since we are starting from an old field, dont write out initial
     !     conditions since it will just be a copy of the old file.
     ! outt0 = .false.    
